...
void AddParticipantAndRedistribute(string name)
{
    lock (DataStore.Lock)
    {
...
app.MapGet("/api/santa/register", () => 
    Results.Json(new { error = "Этот эндпоинт принимает только POST-запросы. Используйте POST с JSON {\"name\":\"Ваше имя\"}" }, 
    statusCode: 405));
